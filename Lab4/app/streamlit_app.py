...
def plot_curves(results: dict, model_name: str) -> plt.Figure:
    ...
    axes[0].plot(epochs_range, results["val_losses"], label="Validation Loss")
    ...
    axes[1].plot(epochs_range, results["train_accs"], label="Train Accuracy")
    axes[1].plot(epochs_range, results["val_accs"], label="Validation Accuracy")
    ...
    axes[2].plot(epochs_range, results["val_aucs"], label="Validation AUC")
...
def plot_confusion_matrix(results: dict) -> plt.Figure:
    ...
    # ax.imshow(...)
    # label axes, add title and ticks
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    if st.button("⚙️ Generate predictions", type="primary"):
        with st.spinner("Running inference on test set…"):
            try:
                st.write(f"DEBUG: sending model_name='{selected_model}'")
