...
class InceptionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.branch2 = nn.Sequential(
            ...
            conv_bn_relu(b2_reduce,   b2_out,    kernel=3, padding=1),  # 3×3 (same spatial)
        )

        # Branch 3: 1×1 → 5×5
        self.branch3 = nn.Sequential(
            conv_bn_relu(in_channels, b3_reduce, kernel=1),       # bottleneck 1×1
            conv_bn_relu(b3_reduce,   b3_out,    kernel=5, padding=2),  # 5×5 (same spatial)
        )

        # Branch 4: 3×3 MaxPool → 1×1 conv
        # FIX: the 1×1 conv after MaxPool was missing — without it the branch
        # outputs in_channels features instead of b4_out, making cat() produce
        # the wrong channel count and breaking every downstream layer.
        self.branch4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1),     # preserves spatial size
            conv_bn_relu(in_channels, b4_out, kernel=1),          # project to b4_out channels
        )
...
class Inception(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, dropout_rate: float = 0.5):
        ...
        # Stem: two conv layers to bring 224×224 down to ~56×56
        ...
        # out_ch = b1 + b2_out + b3_out + b4_out
